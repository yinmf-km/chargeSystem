package com.course.app.common.ext.util;
...
import com.course.app.common.ext.config.CommonExtProperties;
import lombok.Data;
...
public class BizWidgetDatasourceExtHelper {
     ...
     * @return 指定主键Id的数据对象。
     */
    public Map<String, Object> getDataById(String appCode, String type, String id) {
        List<Map<String, Object>> dataList = this.getDataListByIds(appCode, type, id);
        return CollUtil.isEmpty(dataList) ? null : dataList.get(0);
    }

    /**
     * 根据主键集合获取指定通用业务组件的数据对象列表。
     *
     ...
     */
    public List<Map<String, Object>> getDataListByIds(String appCode, String type, String ids) {
        ...
        if (StrUtil.isBlank(appCode)) {
            return this.getDataListByIds(type, ids);
        }
        DatasourceWrapper wrapper = this.getDatasourceWrapper(appCode, type);
        JSONObject body = new JSONObject();
        body.put("type", type);
    ...
    private List<Map<String, Object>> getDataListByIds(String type, String ids) {
        DatasourceWrapper wrapper = this.getDatasourceWrapper(DEFAULT_ORANGE_APP, type);
