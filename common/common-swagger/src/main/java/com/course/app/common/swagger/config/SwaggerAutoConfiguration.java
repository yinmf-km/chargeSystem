...
public class SwaggerAutoConfiguration {
	...
	public Docket upmsDocket(SwaggerProperties properties) {
		return new Docket(DocumentationType.SWAGGER_2).groupName("1.曲靖清源学校缴费系统相关接口")
	...
	public Docket bizDocket(SwaggerProperties properties) {
		return new Docket(DocumentationType.SWAGGER_2).groupName("2. 业务应用分组接口")
				.ignoredParameterTypes(MyRequestBody.class).apiInfo(apiInfo(properties)).select()
	...
	public Docket workflowDocket(SwaggerProperties properties) {
		return new Docket(DocumentationType.SWAGGER_2).groupName("3. 工作流通用操作接口")
				.ignoredParameterTypes(MyRequestBody.class).apiInfo(apiInfo(properties)).select()
