...
import com.course.app.common.core.annotation.RelationGlobalDict;
import com.course.app.common.core.annotation.RelationManyToMany;
import com.course.app.common.core.annotation.RelationOneToMany;
import com.course.app.common.core.annotation.RelationOneToOne;
import com.course.app.common.core.annotation.UploadFlagColumn;
import com.course.app.common.core.exception.InvalidDataFieldException;
...
public class MyModelUtil {
	...
	/**
	 * 拷贝源类型的集合数据到目标类型的集合中，其中源类型和目标类型中的对象字段类型完全相同。 NOTE: 该函数主要应用于框架中，Dto和Model之间的copy，特别针对一对一关联的深度copy。
	...
	/**
	 * 拷贝源类型的对象数据到目标类型的对象中，其中源类型和目标类型中的对象字段类型完全相同。 NOTE: 该函数主要应用于框架中，Dto和Model之间的copy，特别针对一对一关联的深度copy。
	 * 在Dto中，一对一对象可以使用Map来表示，而不需要使用从表对象的Dto。
	 * @param source 源类型对象。
	 * @param targetClazz 目标类型的Class对象。
	 * @param <S> 源类型。
	...
	/**
	 * 映射Model对象的字段反射对象，获取与该字段对应的数据库列名称。 如果没有匹配到ColumnName，则立刻抛出异常。
	 * @param field 字段反射对象。
	 * @param modelClazz Model对象的Class类。
	 * @return 该字段所对应的数据表列名称。
	 */
	...
	/**
	 * 映射Model对象的字段名称，获取与该字段对应的数据库列名称。 如果没有匹配到ColumnName，则立刻抛出异常。
	 * @param fieldName 字段名称。
	 * @param modelClazz Model对象的Class类。
	 * @return 该字段所对应的数据表列名称。
	 */
	...
	/**
	 * 主Model类型中，遍历所有包含RelationConstDict注解的字段，并将关联的静态字典中的数据， 填充到thisModel对象的被注解字段中。
	...
	public static <T> void makeConstDictRelation(Class<T> thisClazz, T thisModel) {
		...
		for (Field field : fields) {
			...
			Map<Object, String> dictMap = (Map<Object, String>) ReflectUtil.getFieldValue(r.constantDictClass(),
					dictMapField);
	...
	/**
	 * 主Model类型中，遍历所有包含RelationConstDict注解的字段，并将关联的静态字典中的数据， 填充到thisModelList集合元素对象的被注解字段中。
	 * @param thisClazz 主对象的Class对象。
	...
	/**
	 * 在主Model类型中，根据thisRelationField字段的RelationDict注解参数，将被关联对象thatModel中的数据， 关联到thisModel对象的thisRelationField字段中。
	 * @param thisClazz 主对象的Class对象。
	 * @param thisModel 主对象。
	 ...
	 * @param <R> 从表对象类型。
	 */
	public static <T, R> void makeDictRelation(Class<T> thisClazz, T thisModel, R thatModel, String thisRelationField) {
	 ...
	 * @param <R> 从表对象类型。
	 */
	public static <T, R> void makeDictRelation(Class<T> thisClazz, List<T> thisModelList, List<R> thatModelList,
	 ...
	 * @param <R> 从表对象类型。
	 */
	public static <T, R> void makeDictRelation(Class<T> thisClazz, List<T> thisModelList, Map<Object, R> thatMadelMap,
			String thisRelationField) {
	 ...
	 * @param <T> 主表对象类型。
	 */
	public static <T> void makeGlobalDictRelation(Class<T> thisClazz, List<T> thisModelList,
			Map<Serializable, String> dictMap, String thisRelationField) {
	 ...
	 * @param <R> 从表对象类型。
	 */
	public static <T, R> void makeOneToOneRelation(Class<T> thisClazz, List<T> thisModelList, List<R> thatModelList,
			String thisRelationField) {
		if (CollUtil.isEmpty(thatModelList) || CollUtil.isEmpty(thisModelList)) {
			return;
		}
	...
	/**
	 * 根据主对象和关联对象各自的关联Id函数，将主对象列表和关联对象列表中的数据关联到一起，并将关联对象 设置到主对象的指定关联字段中。 NOTE: 用于主对象关联字段中，没有包含RelationOneToOne注解的场景。
	 * @param thisClazz 主对象的Class对象。
	 * @param thisModelList 主对象列表。
	 ...
	 * @param <R> 从表对象类型。
	 */
	public static <T, R> void makeOneToOneRelation(Class<T> thisClazz, List<T> thisModelList,
			Function<T, Object> thisIdGetterFunc, List<R> thatModelList, Function<R, Object> thatIdGetterFunc,
			String thisRelationField) {
		makeOneToOneRelation(thisClazz, thisModelList, thisIdGetterFunc, thatModelList, thatIdGetterFunc,
				thisRelationField, false);
	}
	...
	public static <T, R> void makeOneToOneRelation(Class<T> thisClazz, List<T> thisModelList,
			Function<T, Object> thisIdGetterFunc, List<R> thatModelList, Function<R, Object> thatIdGetterFunc,
			String thisRelationField, boolean orderByThatList) {
		if (CollUtil.isEmpty(thisModelList)) {
			return;
		}
		Field thisTargetField = ReflectUtil.getField(thisClazz, thisRelationField);
		...
		if (orderByThatList) {
			...
			Map<Object, ? extends T> thisModelMap = thisModelList.stream()
					.collect(Collectors.toMap(thisIdGetterFunc, c -> c));
			thatModelList.forEach(thatModel -> {
		...
		Map<Object, R> thatMadelMap = thatModelList.stream().collect(Collectors.toMap(thatIdGetterFunc, c -> c));
		thisModelList.forEach(thisModel -> {
	 ...
	 * @param <R> 从表对象类型。
	 */
	public static <T, R> void makeOneToManyRelation(Class<T> thisClazz, List<T> thisModelList, List<R> thatModelList,
			String thisRelationField) {
		if (CollUtil.isEmpty(thatModelList) || CollUtil.isEmpty(thisModelList)) {
			return;
		}
	 ...
	 * @param <R> 关联表对象类型。
	 */
	public static <T, R> void makeManyToManyRelation(Class<T> thisClazz, List<T> thisModelList,
			List<R> relationModelList, String thisRelationField) {
