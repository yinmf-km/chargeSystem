package pmssMpng;

import java.util.UUID;

import com.bocom.api.DefaultBocomClient;
import com.course.app.webadmin.upms.model.pmms.request.MPNG020708RequestV1;
import com.course.app.webadmin.upms.model.pmms.request.ReqHead;
import com.course.app.webadmin.upms.model.pmms.response.MPNG020708ResponseV1;

/**
 * <pre>
 * 预授权完成、撤销
 * </pre>
 * <p>
 * </p>
 */
public class MPNG020708V1Test {

	public static final String MY_PRIVATE_KEY = "";
	public static final String APIGW_PUBLIC_KEY = "";
	public static final String BOCOM_PRD_KEY = "";
	public static final String APP_ID = "app201901170913";
	//	public static final String APIGW_URL_ADDRESS = "http://182.119.88.215:8090";//内网uat
	public static final String APIGW_URL_ADDRESS = "https://117.184.192.242:9443";//外网uat
	//public static final String APIGW_URL_ADDRESS = "http://127.0.0.1:9501";
	//public static final String APIGW_URL_ADDRESS = "https://open.bankcomm.com";//生产地址

	public static void main(String[] args) {
		try {
			DefaultBocomClient client = new DefaultBocomClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
			/**
			 * 测试环境可以忽略SSL证书告警，生产环境不可忽略
			 */
			client.ignoreSSLHostnameVerifier();
			MPNG020708RequestV1 request = new MPNG020708RequestV1();
			request.setServiceUrl(APIGW_URL_ADDRESS + "/api/pmssMpng/MPNG020708/v1");
			MPNG020708RequestV1.MPNG020708RequestV1Biz bizContent = new MPNG020708RequestV1.MPNG020708RequestV1Biz();
			ReqHead reqHead = new ReqHead();
			bizContent.setReqHead(reqHead);
			reqHead.setTransTime("1");
			reqHead.setVersion("1");
			MPNG020708RequestV1.MPNG020708RequestV1Biz.ReqBody reqBody = new MPNG020708RequestV1.MPNG020708RequestV1Biz.ReqBody();
			bizContent.setReqBody(reqBody);
			reqBody.setAmount("1");
			reqBody.setTranScene("1");
			reqBody.setMerPtcId("1");
			reqBody.setNotifyUrl("1");
			reqBody.setMerTradeDate("1");
			reqBody.setShopId("1");
			reqBody.setPartnerId("1");
			reqBody.setPayMerTranNo("1");
			reqBody.setMerMemo("1");
			reqBody.setRefundMerTranNo("1");
			reqBody.setCurrency("1");
			reqBody.setMerRefundTime("1");
			reqBody.setMerRefundDate("1");
			reqBody.setTranContent("1");
			reqBody.setSysOrderNo("1");
			request.setBizContent(bizContent);
			MPNG020708ResponseV1 response = client.execute(request, UUID.randomUUID().toString().replace("-", "1"));
			if (response.isSuccess()) {
				System.out.println("success");
				System.out.println(response.toString());
			} else {
				System.out.println(response.getRspCode());
				System.out.println(response.getRspMsg());
			}
		} catch (Exception xcp) {
			xcp.printStackTrace();
		}
	}
}
