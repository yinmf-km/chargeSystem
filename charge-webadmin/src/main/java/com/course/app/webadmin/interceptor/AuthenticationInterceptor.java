package com.course.app.webadmin.interceptor;
...
import com.course.app.common.core.cache.CacheConfig;
import com.course.app.common.core.constant.ApplicationConstant;
import com.course.app.common.core.constant.DataPermRuleType;
...
import com.course.app.webadmin.config.ThirdPartyAuthConfig;
...
public class AuthenticationInterceptor implements HandlerInterceptor {
	...
	private final ApplicationConfig appConfig = ApplicationContextHolder.getBean("applicationConfig");
	private final ThirdPartyAuthConfig thirdPartyAuthConfig = ApplicationContextHolder.getBean("thirdPartyAuthConfig");
	...
	private boolean handleThirdPartyRequest(String appCode, String token, String url, HttpServletResponse response) {
		...
		TokenData.addToRequest(tokenData);
		if (Boolean.FALSE.equals(tokenData.getIsAdmin()) && !this.hasThirdPartyPermission(authProps, tokenData, url)) {
			response.setStatus(HttpServletResponse.SC_FORBIDDEN);
			this.outputResponseMessage(response, ResponseResult.error(ErrorCodeEnum.NO_OPERATION_PERMISSION));
			return false;
		}
	...
	private ResponseResult<TokenData> getAndCacheThirdPartyTokenData(ThirdPartyAuthConfig.AuthProperties authProps,
			String token) {
		...
		if (responseResult.isSuccess()) {
			sessionData.set(JSON.toJSONString(responseResult.getData()), authProps.getTokenExpiredSeconds(),
					TimeUnit.SECONDS);
		}
	...
	private ResponseResult<TokenData> getThirdPartyTokenData(ThirdPartyAuthConfig.AuthProperties authProps,
			String token) {
		try {
	...
	private ResponseResult<ThirdPartyAppPermData> getThirdPartyPermData(ThirdPartyAuthConfig.AuthProperties authProps,
			String token) {
		try {
	...
	private String invokeThirdPartyUrl(String url, String token) {
		...
		if (!httpResponse.isOk()) {
			String msg = StrFormatter.format("Failed to call [{}] with ERROR HTTP Status [{}] and [{}].", url,
	...
	@SuppressWarnings("unchecked")
	private boolean hasThirdPartyPermission(ThirdPartyAuthConfig.AuthProperties authProps, TokenData tokenData,
			String url) {
		// 为了提升效率，先检索Caffeine的一级缓存，如果不存在，再检索Redis的二级缓存，并将结果存入一级缓存。
		...
		ResponseResult<ThirdPartyAppPermData> responseResult = this.getThirdPartyPermData(authProps,
				tokenData.getToken());
	...
	private void cacheThirdPartyDataPermData(ThirdPartyAuthConfig.AuthProperties authProps, TokenData tokenData,
			List<ThirdPartyAppDataPermData> dataPerms) {
		if (CollUtil.isEmpty(dataPerms)) {
			return;
		}
		Map<Integer, List<ThirdPartyAppDataPermData>> dataPermMap = dataPerms.stream()
				.collect(Collectors.groupingBy(ThirdPartyAppDataPermData::getRuleType));
		...
		for (Map.Entry<Integer, List<ThirdPartyAppDataPermData>> entry : dataPermMap.entrySet()) {
			if (entry.getKey().equals(DataPermRuleType.TYPE_DEPT_AND_CHILD_DEPT)) {
				List<ThirdPartyAppDataPermData> ruleTypeDataPermDataList = normalizedDataPermMap
		...
		for (Map.Entry<Integer, List<ThirdPartyAppDataPermData>> entry : normalizedDataPermMap.entrySet()) {
			if (entry.getKey().equals(DataPermRuleType.TYPE_CUSTOM_DEPT_LIST)) {
				String deptIds = entry.getValue().stream().map(ThirdPartyAppDataPermData::getDeptIds)
	...
	public static class ThirdPartyAppDataPermData {
		...
		/**
		 * 部门Id集合，多个部门Id之间逗号分隔。 注意：仅当ruleType为3、4、5时需要包含该字段值。
		 */
